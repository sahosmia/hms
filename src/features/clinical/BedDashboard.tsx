...
export const BedDashboard: React.FC = () => {
  ...
  const handleAdmissionSubmit = (e: React.FormEvent) => {
    ...
    if (res.success && res.admission) {
      ...
      alert(`Patient admitted successfully! Allocated Bed: '${res.admission.bedName}'`);
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 font-sans space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black text-slate-800">Live Bed Grid (Ward & Room Occupancy)</h2>
          <p className="text-xs text-slate-500 mt-0.5">Allocation, monitoring, and maintenance of ICU, Wards, and Cabins</p>
        </div>
        ...
        {filteredBeds.map((bed: Bed) => {
          ...
          return (
            ...
            >
              ...
              <div className="border-t border-slate-100 pt-2.5 flex justify-between items-center text-xs font-sans">
                <span className="font-bold text-slate-700">BDT {bed.dailyCharge}/day</span>
      ...
      {selectedBedId && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-2xl p-6 space-y-4 shadow-2xl border border-slate-100 animate-slideUp">
            ...
            <div className="flex justify-between items-start">
              <div>
                <h3 className="text-xs font-bold text-slate-400 uppercase">Indoor Admission Wizard</h3>
                <h4 className="text-base font-bold text-slate-800 mt-1">
                  Bed: <span className="text-primary">{beds.find(b => b.id === selectedBedId)?.name}</span>
                </h4>
              </div>
              <button
            ...
            <form onSubmit={handleAdmissionSubmit} className="space-y-4">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs font-semibold text-slate-700">Reason for Admission</label>
                <textarea
                  placeholder="Describe patient health issues or diagnostics brief..."
                  value={reason}
